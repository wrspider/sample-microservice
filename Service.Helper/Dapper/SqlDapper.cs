...
namespace Dapper
{
    ...
    {
        private readonly Connection _connectionOptions;
        private int? commandTimeout = null;
        private DbCurrentType _dbCurrentType;
        private string _connectionString;
        public static IConfiguration? _Configuration { get; private set; }
        public SqlDapper(IOptions<Connection> connectionOptions)
        {
            Connection _connection = _connectionOptions = connectionOptions.Value;
            _connectionString = _connection.DbConnectionString;

        }

        public SqlDapper(Connection _connection)
        {
            _connectionString = _connection.DbConnectionString;
        ...
        public IDbConnection GetDbConnection(string connString = null)
        {
        ...
        public IDbConnection GetDbConnection(string connString = null, DbCurrentType dbCurrentType = DbCurrentType.Default)
        {
            //默认获取DbConnection
        ...
        private T Execute<T>(Func<IDbConnection, IDbTransaction, T> func, bool beginTransaction = false)
        {
            ...
            using (var connection = GetDbConnection(_connectionString, _dbCurrentType))
            {
        ...
        {
            using (_transactionConnection = GetDbConnection(_connectionString, _dbCurrentType))
            {
                try
                {
        ...
        private async Task<T> ExecuteAsync<T>(Func<IDbConnection, IDbTransaction, Task<T>> funcAsync, bool beginTransaction = false)
        {
            ...
            using (var connection = GetDbConnection(_connectionString))
            {
        ...
        {
            //  string sqlStr = KeyValue.GetSqlStr(cmd, param);//仅供调试用的SQL语句
        ...
        /// <returns></returns>
        //public DynamicParameters getQueryParameters(List<SearchParameters> args, DynamicParameters parameters = null)
        //{
        //    if(parameters == null)
        //    {
        ...
        //    if (args != null)
        //    {
        //        var plantCodeParam = args.Find(s => s.Name == "PlantCode");
        //        string plantCode;
        //        if (plantCodeParam != null)
        //        {
        //            plantCode = plantCodeParam.Value;
        //        }
        //        else
        //        {
        //            plantCode = UserContext.Current.UserInfo.DefaultPlantCode;
        //        }
        //        parameters.Add("@PlantCode", plantCode);
        //        foreach (var item in args)
        //        {
        //            var isDate = (item.ValueType != null && item.ValueType.ToLower().Contains("date"));
        //            if (item.DisplayType == "thanorequal")
        //            {
        //                if (isDate && DateTime.TryParse(item.Value,out DateTime dateTime))
        //                {
        //                    parameters.Add($"@{item.Name}_Start", DateTime.Parse(item.Value).ToLocalTime());
        //                }
        //                else
        //                {
        //                    parameters.Add($"@{item.Name}_Start", item.Value);
        //                }
        //            }
        //            else if (item.DisplayType == "lessorequal")
        //            {
        //                if (isDate && DateTime.TryParse(item.Value, out DateTime dateTime))
        //                {
        //                    parameters.Add($"@{item.Name}_End", DateTime.Parse(item.Value).ToLocalTime());
        //                }
        //                else
        //                {
        //                    parameters.Add($"@{item.Name}_End", item.Value);
        //                }
        //            }
        //            else
        //            {
        //                if (item.Values == null)
        //                {
        //                    if (isDate && DateTime.TryParse(item.Value,out DateTime date))
        //                    {
        //                        parameters.Add($"@{item.Name}", DateTime.Parse(item.Value).ToLocalTime());
        //                    }
        //                    else
        //                    {
        ...
        //                else
        //                {
        //                    var result = new object[item.Values.Length];
        //                    for (var i = 0; i < item.Values.Length; i++)
        //                    {
        //                        if (isDate && DateTime.TryParse(item.Values[i],out DateTime date))
        //                        {
        //                            result[i] = DateTime.Parse(item.Values[i]).ToLocalTime();
        //                        }
        //                        else
        //                        {
        //                            result[i] = item.Values[i];
        //                        }
        //                    }
        //                    parameters.Add($"@{item.Name}", result);
        //                }
        //            }
        //        }
        //    }
        ...
        /// <returns></returns>
        public string AddConditionForSqlString(string sql, SearchParameters item, string paramName, string aliasName, string paramValue = null)
        {
            if (item == null)
            {
                sql += $" AND {aliasName}.{paramName} = '{paramValue}'";
                return sql;
            }
            if (item.Name == paramName)
            {
                if (!string.IsNullOrEmpty(item.Value) && item.DisplayType == HtmlElementType.like)
                {
                    // like
                    sql += $" AND {aliasName}.{paramName} like concat('%',@{paramName},'%')";
                }
                else if (item.Values != null && item.Values.Count() > 0 && !string.IsNullOrEmpty(item.Values[0]))
                {
                    // 多选
                    sql += $" AND {aliasName}.{paramName} in @{paramName}";
                }
                else if (!string.IsNullOrEmpty(item.Value))
                {
                    // 单选
                    sql += $" AND {aliasName}.{paramName} = @{paramName}";
                }
            }
            return sql;
        }
        ...
        /// <param name="paramValue"></param>
        /// <returns></returns>
        public string AddDeleteConditionForSqlString(string sql, string[] aliasList, string paramName = null, string paramValue = null)
        {
            foreach (var alias in aliasList)
            {
                sql += $" AND ({alias}.ISDELETE != 'Y' or {alias}.ISDELETE is null)";
            }
            return sql;
        }
        ...
        public static string GetSqlStr(string sql, object param = null)
        {
            try
            {
                ...
                if (param != null)
                {
                    ...
                    if (typeName == "DynamicParameters")
                    {
                        DynamicParameters parameters = (DynamicParameters)param;
                        arr = parameters.ParameterNames.ToArray();
                    ...
                    {
                        arr = JsonConvert.SerializeObject(param).Replace("{", "").Replace("}", "").Split(',');
                        foreach (var item in arr)
                        {
                            KeyValue kv = new KeyValue();
                            string[] temp = item.Split(':');
                            kv.Key = temp[0].Replace("\"", "").Trim();
                            kv.Value = temp[1].Replace("\"", "").Trim();
                            paramList.Add(kv);
                        }
                    }
